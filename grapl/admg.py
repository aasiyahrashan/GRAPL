...
class ADMG():
    ...
    def delete_incoming(self, nodes2delete={}):
        for node2delete in nodes2delete:
            self.vars[node2delete].parents = set()
            self.vars[node2delete].bidirects = set()

            for node in set(self.vars):
                self.vars[node].children.discard(node2delete)
                self.vars[node].bidirects.discard(node2delete)

    def delete_outgoing(self, nodes2delete):
        for node2delete in nodes2delete:
            self.vars[node2delete].children = set()
            self.vars[node2delete].bidirects = set()

            for node in set(self.vars):
                self.vars[node].parents.discard(node2delete)
                self.vars[node].bidirects.discard(node2delete)
