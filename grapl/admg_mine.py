...
class ADMG():
    ...
    def deledges(self, name={}, type = {'parents', 'children'}):
        """Remove arrows connecting a variable to either it's parents or children.
           Edges are Sets of Strings, each string representing a random variable in the graph.
        """
        if name != '':
            if type == 'parents':
                self.vars[name].parents = {}

            elif type == 'children':
                # Need to delete both the children of the variable and the connection between the children and the original variable.
                children = self.vars[name].children
                if len(children) > 0:
                # Deleting the parent connection from children of the variable
                    for ch in children:
                        print(ch)
                        self.vars[ch].parents.remove(name)
                        print(self.vars[ch].parents)
                    # Deleting the child connections
                    self.vars[name].children = {}
        else:
            raise ValueError('admg:deledges: Invalid node name')

        ### This shouldn't make any changes if the above has worked.
        self.connect()

        ### I feel returning this is bad but am not sure what else to do.
        return self
